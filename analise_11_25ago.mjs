...
const PERIODO_INICIO = new Date("2026-08-11T00:00:00-03:00");
const PERIODO_FIM    = new Date("2026-08-25T23:59:59-03:00");
...
function horaMensagemNoPeriodo(s) {
  ...
  if (!mes) return false;
  const dt = new Date(2026, mes - 1, +m[1], 12, 0, 0);
...
console.log(`Análise: 11/ago → 25/ago/2026 (15 dias)`);
...
const out = {
  periodo: { inicio: "2026-08-11", fim: "2026-08-25", dias: DIAS_PERIODO },
...
writeFileSync("relatorio_11_25ago.json", JSON.stringify(out, null, 2));

console.log("\n" + "=".repeat(60));
console.log(`RESULTADOS 11/ago → 25/ago/2026 (15 dias)`);
console.log("=".repeat(60));
...
console.log(`\nDisparos/dia:`);
for (const [k,v] of Object.entries(dispDia).sort((a,b)=>b[1]-a[1])) console.log(`  ${k}: ${v}`);
console.log(`\nTransferências por dia:`);
for (const [k,v] of Object.entries(tDia).sort()) console.log(`  ${k}: ${v}`);
console.log(`\nPor vendedor:`);
...
console.log(`\nRelatório salvo: relatorio_11_25ago.json`);
